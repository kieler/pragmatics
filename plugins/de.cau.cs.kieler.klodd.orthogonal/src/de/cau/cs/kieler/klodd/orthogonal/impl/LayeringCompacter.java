...
/**
 * Compacter implementation that performs fast compaction for refined orthogonal
...
public class LayeringCompacter extends AbstractAlgorithm implements ICompacter {
    ...
    /**
     * Structure describing a horizontal or vertical bar used for topological
     * numbering.
     */
    ...
    /**
     * Returns the first endpoint of an edge that is incident at the given side
     * for the given node. For a normalized graph there is at most one such
     * node.
     * 
     * @param node the node
     * @param side the side
    ...
    /**
     * Visit a bar: determine its rank by finding the longest path to the single
     * source bar.
     * 
     * @param topoBar bar to process
     * @param direction direction to go for the source
     */
    private void visit(final TopoBar topoBar, final KSlimNode.Side direction) {
        int minRank = 0;
