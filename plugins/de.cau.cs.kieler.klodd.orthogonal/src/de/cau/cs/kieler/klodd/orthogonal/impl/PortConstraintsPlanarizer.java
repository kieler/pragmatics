...
public class PortConstraintsPlanarizer extends AbstractAlgorithm implements IPlanarizer {
    ...
    /**
     * Creates a port constraints planarizer with a given embedding constraints
     * planarizer.
     * 
     * @param theecPlanarizer EC planarizer
     */
    public PortConstraintsPlanarizer(final IPlanarizer theecPlanarizer) {
        this.ecPlanarizer = theecPlanarizer;
    }
                            ...
                            case NORTH:
                                if (northConstraint == null) {
                                    northConstraint = new EmbeddingConstraint(
                                            EmbeddingConstraint.Type.GROUPING, sideConstraint, null);
                                }
     ...
     * @return a constraint for the given port, or null if the port has no
     *         incoming or outgoing edges
     */
    private EmbeddingConstraint createConstraintFor(final KPort port,
        ...
        if (!portEdges.isEmpty()) {
            ...
            for (TSMEdge edge : portEdges) {
                ...
                EmbeddingConstraint edgeConstraint = new EmbeddingConstraint(constraintType,
                        groupConstraint, edge);
