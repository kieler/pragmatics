...
public class ConnectedComponents extends AbstractAlgorithm {
    ...
    /**
     * Finds the connected components of a layout graph and returns them as
     * separate TSM graphs.
     * 
     * @param parentNode parent layout node
     * @return list of connected components
     */
    public List<TSMGraph> findComponents(final KNode parentNode) {
     ...
     */
    private TSMGraph findComponent(final KNode startLayoutNode) {
        ...
        while (!edgeStack.isEmpty()) {
            ...
            if (layoutEdge.getSource().getParent() == layoutEdge.getTarget().getParent()) {
                if (!edgeMap.containsKey(layoutEdge)) {
                    ...
                    if (sourceNode == null) {
                        sourceNode = new TSMNode(graph, TSMNode.Type.LAYOUT, currentLayoutNode);
                    ...
                    if (targetNode == null) {
                        targetNode = new TSMNode(graph, TSMNode.Type.LAYOUT, currentLayoutNode);
                        nodeMap.put(currentLayoutNode, targetNode);
                        edgeStack.addAll(currentLayoutNode.getOutgoingEdges());
                        edgeStack.addAll(currentLayoutNode.getIncomingEdges());
                    }
                    KSlimEdge tsmEdge = new TSMEdge(graph, sourceNode, targetNode, layoutEdge);
