...
public class ConstraintExpander extends AbstractAlgorithm {
    ...
    /**
     * Creates a TSM graph from a given slim graph and expands all
     * embedding constraints.
     * 
     * @param graph slim graph to process
    ...
    /**
     * Creates a new node for the given node of the input graph and registers
     * all incident edges.
     * 
    ...
    /**
     * Expands a given embedding constraint and connects the expansion with the
     * given node.
     * 
     * @param constraint embedding constraint to expand
     * @param parentNode node to connect, or null if there is none
     */
    private void expandConstraint(final EmbeddingConstraint constraint, final TSMNode parentNode) {
        ...
        case IN_EDGE:
            ...
            registerEdge(parentNode, edge, constraint.type == EmbeddingConstraint.Type.OUT_EDGE);
            break;
        case GROUPING:
            // create a single node and connect children with it
            TSMNode groupingNode = new TSMNode(expandedGraph, TSMNode.Type.ECEXPANSION, constraint);
            if (parentNode != null) {
        ...
        case MIRROR:
            ...
            KSlimNode hubNode = new TSMNode(expandedGraph, TSMNode.Type.ECEXPANSION, constraint);
            KSlimNode firstNode = null,
            lastNode = null;
            if (parentNode != null) {
     ...
     * @param forward if true, the edge goes out of the given node, else it
     *            comes into the given node
     */
    private void registerEdge(final TSMNode node, final TSMEdge edge, final boolean forward) {
        if (forward) {
