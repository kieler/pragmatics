 ...
 */
package de.cau.cs.kieler.klay.cola.algs;
 ...
 * <ul>
 *   <li>Peter Eades, Xuemin Lin, W. F. Smyth, A fast and effective heuristic for the feedback arc set
 *     problem. <i>Information Processing Letters</i> 47(6), pp. 319-323, 1993.
 *   </li>
 *   <li>Giuseppe di Battista, Peter Eades, Roberto Tamassia, Ioannis G. Tollis, <i>Graph Drawing:
 *     Algorithms for the Visualization of Graphs</i>, Prentice Hall, New Jersey, 1999 (Section 9.4).
 *   </li>
 * </ul>
 * 
 * @author msp
 ...
 * 
 */
public final class EadesMFASHeuristic {
    ...
    /** a set of the nodes to perform fast containment checking. */
    private Set<CNode> nodeSet;
    
    /**
     * @param nodes
     *            the nodes for which to apply this heuristic.
     */
    private EadesMFASHeuristic(final List<CNode> nodes) {
        this.nodes = nodes;
     ...
     * @param nodes
     *            the nodes for which to apply this heuristic.
     * @return the set of edges that have to be reversed to make the passed set of nodes acyclic.
     */
    public static Set<CEdge> execute(final List<CNode> nodes) {
        return new EadesMFASHeuristic(nodes).process();
    }

    /**
     * @return the set of edges that have to be reversed to make the passed set of nodes acyclic.
     */
    private Set<CEdge> process() {
        if (nodes.size() == 1) {
        ...
        while (unprocessedNodeCount > 0) {
            // sinks are put to the right --> assign negative rank, which is later shifted to
            // positive
            while (!sinks.isEmpty()) {
    ...
    /**
     * Updates indegree and outdegree values of the neighbors of the given node, simulating its
