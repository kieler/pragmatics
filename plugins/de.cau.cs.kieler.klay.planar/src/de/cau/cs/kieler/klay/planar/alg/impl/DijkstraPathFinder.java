...
import de.cau.cs.kieler.klay.planar.alg.IShortestPathFinder;
...
public class DijkstraPathFinder extends AbstractAlgorithm implements IShortestPathFinder {
     ...
     * {@inheritDoc}
     */
    public List<IEdge> findShortestPath(final INode source, final INode target) {
        ...
        while (!nodes.isEmpty()) {
            ...
            for (IEdge edge : current.adjacentEdges()) {
                ...
                Integer cost = edge.getProperty(IShortestPathFinder.EDGECOST);
                if (cost == null) {
                    cost = 1;
                }
