...
public class CustomBorderItemLocator implements IBorderItemLocator {
	...
	private Point getFixedLocation(int side, IFigure borderItem){
		int x=0,y=0;
		Rectangle parentBounds = getParentBorder();
		Rectangle childBounds = borderItem.getBounds();
		
		switch (side) {
		// FIXME: side is always SOUTH...
		case PositionConstants.SOUTH_EAST:
			x = parentBounds.x + parentBounds.width - childBounds.width;
			if(x < parentBounds.x)
				x = parentBounds.x;
			y = parentBounds.y + parentBounds.height + getBorderItemOffset().height;
			break;
		case PositionConstants.SOUTH_WEST:
			x = parentBounds.x;
			if( (x + childBounds.width) > (parentBounds.x + parentBounds.width) )
				x = parentBounds.x + parentBounds.width - childBounds.width;
			y = parentBounds.y + parentBounds.height + getBorderItemOffset().height;
			break;
		case PositionConstants.SOUTH:
			x = parentBounds.x + (parentBounds.width/2) - (childBounds.width/2);
			y = parentBounds.y + parentBounds.height + getBorderItemOffset().height;
			break;
...
//		setCurrentSideOfParent(findClosestSideOfParent(new Rectangle(ptNewLocation, size), getParentBorder()));
        Point newLocation = getFixedLocation(this.preferredSide, borderItem);
