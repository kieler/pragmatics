...
public class OgdfLayoutProvider extends AbstractLayoutProvider {
    ...
    public synchronized void doLayout(final KNode layoutNode,
        ...
        final IPreferenceStore preferenceStore =
                OgdfPlugin.getDefault().getPreferenceStore();
        final Maybe<Boolean> docontinue = new Maybe<Boolean>();
        docontinue.set(true);
        if (!preferenceStore.getString(PREF_EXPERIMENTAL).equals(
        ...
        if (docontinue.get()) {
            ...
            for (KNode child : layoutNode.getChildren()) {
                ...
                LayoutOptions.setBoolean(nodeLayout, LayoutOptions.NO_LAYOUT,
                        true);
                for (KPort port : layoutNode.getPorts()) {
                    KShapeLayout portLayout =
                            KimlLayoutUtil.getShapeLayout(port);
                    LayoutOptions.setBoolean(portLayout,
                ...
                for (KEdge edge : layoutNode.getOutgoingEdges()) {
                    ...
                    LayoutOptions.setBoolean(edgeLayout,
                            LayoutOptions.NO_LAYOUT, true);
                    for (KLabel label : edge.getLabels()) {
                        KShapeLayout labelLayout =
                                KimlLayoutUtil.getShapeLayout(label);
                        LayoutOptions.setBoolean(labelLayout,
                                LayoutOptions.NO_LAYOUT, true);
                    }
                }
            }
