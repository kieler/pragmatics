...
public class GraphsCreationWizard extends Wizard implements INewWizard {
	...
	public void addPages() {
		diagramModelFilePage = new GraphsCreationWizardPage(
				"DiagramModelFile", getSelection(), "graphs_diagram"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		domainModelFilePage = new GraphsCreationWizardPage(
				"DomainModelFile", getSelection(), "graphs") { //$NON-NLS-1$ //$NON-NLS-2$

			public void setVisible(boolean visible) {
				if (visible) {
					String fileName = diagramModelFilePage.getFileName();
					fileName = fileName.substring(0, fileName.length()
							- ".graphs_diagram".length()); //$NON-NLS-1$
					setFileName(GraphsDiagramEditorUtil.getUniqueFileName(
							getContainerFullPath(), fileName, "graphs")); //$NON-NLS-1$
				}
	...
	public boolean performFinish() {
		IRunnableWithProgress op = new WorkspaceModifyOperation(null) {
			...
			protected void execute(IProgressMonitor monitor)
				...
				diagram = GraphsDiagramEditorUtil.createDiagram(
