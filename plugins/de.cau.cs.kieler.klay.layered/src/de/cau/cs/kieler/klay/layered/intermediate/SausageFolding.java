...
public class SausageFolding implements ILayoutProcessor {
    ...
    public void process(final LGraph graph, final IKielerProgressMonitor progressMonitor) {
        ...
        spacing = graph.getProperty(Properties.OBJ_SPACING);
        ...
        double maxHeight = determineMaximalHeight(graph); // sum of nodes in the layer
        ...
        double currentAR = (maxHeight / sumWidth);
        
        // get the number of rows into which to split the sausage
        double rows = Math.max(1, desiredAR / currentAR - 1); 
