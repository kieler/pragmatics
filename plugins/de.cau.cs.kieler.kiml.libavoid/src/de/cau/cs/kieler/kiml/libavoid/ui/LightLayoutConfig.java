...
/**
 * Layout configurator for a special libavoid layout.
 * 
 * The configuration is derived from widgets created by the {@link LayoutOptionControlFactory}.
 * 
 * Something to remind: The optionsMap holds {@link IProperty} elements and is queried for
 * {@link LayoutOptionData} in the {@link #getValue(LayoutOptionData, LayoutContext)} method. This
 * works because both classes use the id's for hashCode and equals.
 * 
 * @author uru
 * @author msp
 */
public class LightLayoutConfig implements ILayoutConfig {
    ...
    private final Map<IProperty<?>, Object> optionMap = Maps.newHashMap();
     ...
     * 
     * @param optionData
     *            a layout option data instance
     * @param value
     *            the new value for the option
     */
    public void setOption(final IProperty<?> optionData, final Object value) {
     ...
     */
    public Object getOption(final IProperty<?> optionData) {
    ...
    public void transferValues(final KLayoutData graphData, final LayoutContext context) {
        Object diagramPart = context.getProperty(LayoutContext.DIAGRAM_PART);
        for (Map.Entry<IProperty<?>, Object> entry : optionMap.entrySet()) {
            LayoutOptionData<?> optionData =
                    LayoutDataService.getInstance().getOptionData(entry.getKey().getId());
            if (matchesTargetType(optionData, diagramPart)) {
     ...
     */
    private boolean matchesTargetType(final LayoutOptionData<?> optionData, final Object diagramPart) {
