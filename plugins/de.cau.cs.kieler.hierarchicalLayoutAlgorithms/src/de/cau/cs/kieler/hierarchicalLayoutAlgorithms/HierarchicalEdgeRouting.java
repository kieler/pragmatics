...
public class HierarchicalEdgeRouting {
	...
	public static void routeEdgesParentToChild(KNode graph) {
		...
		for (KNode child : graph.getChildren()) {
			...
			int id = 0;//shapeChild.getProperty(HierarchicalMetaDataProvider.HIERARCHICAL_I_D);

			KNode copyOfChild = null;
			for (KNode successor : copiedSuccessors) {
				KShapeLayout shape = successor.getData(KShapeLayout.class);
				int parent_id = shape.getProperty(HierarchicalMetaDataProvider.HIERARCHICAL_I_D);
				if (id == parent_id) {
					...
					KPoint targetPoint = KLayoutDataFactory.eINSTANCE.createKPoint();
					targetPoint.setX(shape.getXpos() + shape.getWidth() / 2);
					targetPoint.setY(shape.getYpos() + shape.getHeight() / 2);
