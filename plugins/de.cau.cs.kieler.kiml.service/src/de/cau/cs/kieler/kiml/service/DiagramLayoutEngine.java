...
public class DiagramLayoutEngine {
     ...
     * <br>
     * Such {@link ILayoutCancelationHandle ILayoutCancelationHandles} can be employed when invoking
     * automatic layout via {@link DiagramLayoutEngine#layout(
     * IWorkbenchPart, Object, ILayoutCancelationHandle, ILayoutConfig...)}.
     * 
     * @author chsch
     */
    public interface ILayoutCancelationHandle {
     ...
     * @return
     */
    private static boolean isCanceled(final ILayoutCancelationHandle handle) {
        return handle != null && handle.isCanceled();
    }
     ...
     * Use a {@link VolatileLayoutConfig} to configure layout options:
     * <pre>
     * DiagramLayoutEngine.INSTANCE.layout(workbenchPart, diagramPart, cancellationHandle,
     *          new VolatileLayoutConfig()
     *              .setValue(LayoutOptions.ALGORITHM, "de.cau.cs.kieler.klay.layered")
     *              .setValue(LayoutOptions.SPACING, 30.0f)
     *              .setValue(LayoutOptions.ANIMATE, true));
     ...
     *            the parent diagram part for which layout is performed, or {@code null} if the whole
     *            diagram shall be layouted
     * @param cancelationHandle
     *            an {@link ILayoutCancelationHandle} evaluated repeatedly during the layout
     ...
     */
    public LayoutMapping<?> layout(final IWorkbenchPart workbenchPart, final Object diagramPart,
            final ILayoutCancelationHandle cancelationHandle, final ILayoutConfig... configurators) {
        ...
        if (layoutManager != null) {
            final LayoutMapping<?> mapping =
                    layout(layoutManager, workbenchPart, diagramPart, cancelationHandle, configurators);
            if (mapping != null) {
    ...
    protected <T> LayoutMapping<T> layout(final IDiagramLayoutManager<T> layoutManager,
            ...
            final ILayoutCancelationHandle cancelationHandle,
            final ILayoutConfig... configurators) {
        ...
        final MonitoredOperation monitoredOperation = new MonitoredOperation(executorService) {
            ...
            class CancelableProgressMonitorEx extends CancelableProgressMonitor {
                ...
                public boolean isCanceled() {
                    ...
                    } else if (DiagramLayoutEngine.isCanceled(cancelationHandle)) {
            ...
            protected void preUIexec() {
                if (isCanceled(cancelationHandle)) {
                    this.cancel();
                    return;
                }
