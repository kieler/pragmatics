 ...
 * @author swe
 */
public abstract class ServiceModelLayoutDataService extends ExtensionLayoutDataService {
     ...
     * 
     */
    protected ServiceModelLayoutDataService() {
    }
    ...
    protected void initializeFromString(final String serializedData) {        
        try {
            ...
            ServiceData serviceMeta = transformer.deserialize(serializedData);
            Vector<IConfigurationElement> vectorElements = new Vector<IConfigurationElement>();
            for (Category category : serviceMeta.getCategories()) {   
                vectorElements.add(ServiceModelConfigurationElement.getCategoryElementFromModel(category));
            }
            for (LayoutType type : serviceMeta.getLayoutTypes()) {                
                vectorElements.add(ServiceModelConfigurationElement.getLayoutTypeElementFromModel(type));
            }
            for (LayoutOption option : serviceMeta.getLayoutOptions()) {
                vectorElements.add(ServiceModelConfigurationElement.getLayoutOptionElementFromModel(option));
            }
            for (LayoutAlgorithm algorithm : serviceMeta.getLayoutAlgorithms()) {
                vectorElements.add(ServiceModelConfigurationElement.getLayoutAlgorithmElementFromModel(algorithm));
            }
