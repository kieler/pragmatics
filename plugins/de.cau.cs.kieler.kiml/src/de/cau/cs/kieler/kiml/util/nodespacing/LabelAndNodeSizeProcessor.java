...
public class LabelAndNodeSizeProcessor {
    ...
    public void process(final GraphAdapter<?> layeredGraph) {
        ...
        for (NodeAdapter<?> node : layeredGraph.getNodes()) {
            ...
            /* PREPARATIONS
             * Reset stuff and remember the node's old size.
             */
            resetContext();
             ...
             * final position of ports and of node labels to calculate all this stuff.
             */
    ...
    /**
     * Places the labels of the given port on the outside of the port's node. We suppose that the first
     * label has label side information. Those are then used for all labels. We don't support having
     * some labels above and others below incident edges.
     * 
    ...
    private void placePortLabelsOutside(final PortAdapter<?> port, final double labelSpacing) {
        ...
        // labels downwards. Of they are placed above or if we have a nothern port, however, we actually
    ...
    /**
     * Calculates the port's margins such that its labels are part of them and sets them accordingly.
     * 
    ...
    private void calculateAndSetPortMargins(final PortAdapter<?> port) {
        ...
        Iterable<LabelAdapter<?>> labels = port.getLabels();
     ...
     */
    private void calculateRequiredNodeLabelSpace(final NodeAdapter<?> node, final double labelSpacing) {
    ...
    private void resizeNode(final NodeAdapter<?> node, final double portSpacing,
        ...
        if (sizeConstraint.contains(SizeConstraint.MINIMUM_SIZE)) {
            ...
            if (sizeOptions.contains(SizeOptions.DEFAULT_MINIMUM_SIZE)) {
                if (minWidth <= 0) {
                    minWidth = 20; // SUPPRESS CHECKSTYLE MagicNumber
                }
                
                if (minHeight <= 0) {
                    minHeight = 20; // SUPPRESS CHECKSTYLE MagicNumber
                }
            }
    ...
    /**
     * Calculate how much space the ports will require at a minimum if they can be freely distributed on
     * their respective node side. The x coordinate of the returned vector will be the minimum width
     * required by the ports on the northern and southern side. The y coordinate, in turn, will be the
     * minimum height required by the ports on the western and eastern side. This may include the space
     * required for port labels placed outside of the node. If port labels are placed inside, their
    ...
    private KVector calculatePortSpaceRequirements(final NodeAdapter<?> node, final double portSpacing,
        ...
        // Calculate the required width and height, taking the necessary spacing between (and around)
    ...
    private void placeNodePorts(final NodeAdapter<?> node) {
        ...
        PortPlacementData placementData = computePortPlacementData(node);
    ...
    private PortPlacementData computePortPlacementData(final NodeAdapter<?> node) {
        ...
        if (additionalPortSpace == null) {
            ...
            double usableHeight = nodeSize.y - additionalPortSpace.top - additionalPortSpace.bottom;
            double usableWidth = nodeSize.x - additionalPortSpace.left - additionalPortSpace.right;
            
            // Calculate how many gaps we have between ports (this is usually one less than the number of
            ...
            data.eastY = additionalPortSpace.top
                    + (eastPortsCount == 1 ? data.eastGapSize : 0);
            data.northX = additionalPortSpace.left
                    + (northPortsCount == 1 ? data.northGapSize : 0);
    ...
    private static final class PortPlacementData {
        // The number of gaps between the ports (this is usually one less than the number of ports we
