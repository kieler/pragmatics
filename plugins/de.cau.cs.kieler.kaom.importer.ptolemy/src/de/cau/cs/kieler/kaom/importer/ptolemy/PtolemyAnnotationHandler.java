...
public class PtolemyAnnotationHandler extends TransformationWorkflowHook {
    ...
    private PropertyType findOriginalProperty(final Annotation annotation) {
        ...
        EObject currentElement = inputRoot;
     ...
     */
    @Override
    protected void invokeInternal(final WorkflowContext ctx, final ProgressMonitor monitor,
            final Issues issues) {
        
        // Obtain the unknown features of the input file
        for (Resource resource : getReader().getResourceSet().getResources()) {
            if (resource instanceof XMLResource) {
                unknownFeatures.putAll(((XMLResource) resource).getEObjectToExtensionMap());
            }
        }
        
        // Fetch the input model
        Object o = ctx.get(this.getInputModelSlot());
        if (o instanceof DocumentRoot) {
            inputRoot = (DocumentRoot) o;
        } else {
            issues.addError(this, "Couldn't find root entity of input model.", o);
            return;
        }
        
        // Fetch the model being transformed
        o = ctx.get(this.getOutputModelSlot());
        if (o instanceof Entity) {
            traverseModelTree((Entity) o);
        } else {
            issues.addError(this, "Model root is not an instance of Entity.", o);
            return;
        }
    }
}
