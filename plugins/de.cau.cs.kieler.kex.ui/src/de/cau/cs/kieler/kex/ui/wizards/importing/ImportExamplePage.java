...
/**
 * This class represents the import page of importwizard. It contains a tree
 * which shows the importable examples. Secondly there is a description and a
 * preview picture field. The {@link WizardResourceImportPage} is extended
 * because of getting a Workspace Directory Chooser.
 * 
 * @author pkl
 * 
 */
public class ImportExamplePage extends WizardResourceImportPage {
	...
	private void createBottomComponent(final Composite parent) {
		...
		this.exampleDescription = new Text(parent, SWT.NONE | SWT.MULTI
	...
	private Control createTree(final Composite composite,
		...
		if (categories.size() == 0) {
			MessageDialog
					.openError(getShell(),
							"Could not start example import wizard",
	...
	/**
	 * loads preview image. The parameters define the max width and heigt of an
	 * image. The loaded image will scaled to the parameter values, while
	 * keeping the imageformat.
	 * 
	 * @param image_width
	 * @param image_height
	 * @return
	...
	private void updateDescriptionLabel(final Example example) {
		...
		sb.append("Title: ").append(example.getTitle()).append("\n")
				.append("Author: ").append(example.getAuthor()).append("\n")
				.append("Contact: ").append(example.getContact()).append("\n")
	...
	private Image initPreviewImage() {
		Image preview = new Image(previewComp.getDisplay(), IMAGE_PRE_WIDTH,
				IMAGE_PRE_HEIGHT);
