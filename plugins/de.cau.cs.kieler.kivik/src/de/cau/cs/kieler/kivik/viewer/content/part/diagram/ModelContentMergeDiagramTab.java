 ...
 * <p/>
 * Integrates as a new tab ContentMerge viewer. Provides functionality to navigate easily to the
 * differences in the two diagrams via clicking in the diagram or by selecting an entry in the
 * separate StructureMergeViewer. Zooming and scrolling is done automatically, but the behavior can
 * be adjusted through the preference page.
 * <p/>
 * No means to merge diagrams yet, but should be possible as the diagrams are GMF diagrams and EMF
 * can merge those models.
 * 
 * @author ars
 * @see ModelStructureMergeViewer
 */
public class ModelContentMergeDiagramTab extends DiagramGraphicalViewer implements
        IModelContentMergeViewerTab, IPropertyChangeListener {
    ...
    /**
     * Creates a new Diagram meger tab extending a DiagramGraphicalViewer under the given parent
     * control.
     * 
    ...
    public ModelContentMergeDiagramTab(Composite parentComposite, int side,
        ...
        KivikPlugin.getDefault().getPreferenceStore().addPropertyChangeListener(this);

        /* initialize the DiagramGraphicalViewer */
        setEditDomain(new EditDomain());
        setEditPartFactory(EditPartService.getInstance());
        setRootEditPart(new RenderedDiagramRootEditPart(MeasurementUnit.PIXEL_LITERAL));
        ...
        /*
         * create and add a new scalable feedback layer to the viewer to display the highlighting
         */
    ...
    public void dispose() {
        ...
        KivikPlugin.getDefault().getPreferenceStore().removePropertyChangeListener(this);
    }

    /**
     * Sets the input for the Diagram viewer and marks the differences from this input in contrast
     * to the input in the opposite viewer.
     * 
     * @param object
     *            The object which should be displayed, Diagram or EObject with a resource
    ...
    public void setReflectiveInput(Object object) {
        ...
        if (prefRelayoutDiagram || collapsedElements) {
            ...
            /*
             * force the diagram layouter to perform several layout steps, as otherwise the
    ...
    /**
     * Takes a diagram as input and closes all the unchanged compartments. The changes are read out
     * of the global map of changes which must be built up before.
     * <p/>
     * Collapsing is done by by setting the AnimateableScrollPane of the compartments to collapsed.
     * 
     * @param diagram
    ...
    private boolean collapseUnchanged(Diagram diagram) {
        ...
        while (allContents.hasNext()) {
            ...
            if (next instanceof View) {
                ...
                DiffElement theChange = dataToRecursivelyDiff.get(theView.getElement());
    ...
    /**
     * Colorizes all the changed EditParts in the Diagram with the respective color.
     * <p/>
     * Changes are read out with another function which relies on the global registry of the
     * changes, so that must be done first.
     */
        ...
        for (ModelContentMergeTabObject object : getVisibleElements()) {
            if (object.getActualObject() instanceof AbstractGraphicalEditPart) {
                ...
                if (changedEditPart instanceof ConnectionEditPart) {
                    changedEditPart.getFigure().setForegroundColor(brightenColor(highlightColor));
                } else {
                    /*
                     * If a compartment is collapsed, then highlight the whole container
                     */
                    if (changedEditPart.getParent() != null
                            && changedEditPart.getParent() instanceof AbstractGraphicalEditPart) {
                        if (((AbstractGraphicalEditPart) changedEditPart.getParent()).getFigure() instanceof ResizableCompartmentFigure) {
                            ...
                            if (!asp.isExpanded()) {
                                ...
                                parentFigure.getParent().setBackgroundColor(highlightColor);
                            }
                        }
                    }
                    changedEditPart.getContentPane().setBackgroundColor(highlightColor);
                }
                if (changedEditPart.getChildren().size() > 0
                    ...
                    ShapeCompartmentEditPart scep = (ShapeCompartmentEditPart) changedEditPart
                            .getChildren().get(0);
                    scep.getFigure().setToolTip(getToolTip(changedEditPart, highlightColor));
                } else {
    ...
    /**
     * Maps the input's differences if any. That is map a DiffElement to an EditPart.
     */
    private void mapDifferences() {
        ...
        while (diffIterator.hasNext()) {
            ...
            if (diffContainer instanceof DiffGroup) {
                dataContainer = EMFCompareEObjectUtils.getLeftElement(((DiffGroup) diffContainer));
    ...
    /**
     * Maps also the DiffGroups to EditParts. That are then EditParts with content. Is done to mark
     * also EditParts whose contained EditParts have changed as changed. But they are put in another
     * HashMap as they should not be colorized or merged (maybe in a later implementation).
     */
    private void mapDiffGroups() {
        dataToRecursivelyDiff.clear();
        final TreeIterator<EObject> diffGroupIterator = parent.getDiffModel().eAllContents();
        while (diffGroupIterator.hasNext()) {
            ...
            if (next instanceof DiffGroup) {
                ...
                if (data != null) {
                    ...
                    if (matchElements instanceof Match2Elements) {
                        if (partSide == EMFCompareConstants.LEFT) {
                            dataToRecursivelyDiff.put(((Match2Elements) matchElements)
                ...
                if (next instanceof DiffElement) {
                    ...
                    if (partSide == EMFCompareConstants.LEFT
                            && !dataToRecursivelyDiff.containsKey(EMFCompareEObjectUtils
    ...
    /**
     * This will map all the EditParts in this DiagramGraphicalViewer that need be taken into
     * account when drawing diff markers to a corresponding ModelContentMergeTabObject. This will
     * allow us to browse everything once and for all.
     * <p/>
     * The markers are not drawn in the Diagram compare view, as in the case when having the
     * Differences or Properties tab enabled.
     */
    private void mapDiffToTabObjects() {
        ...
        for (EObject key : dataToDiff.keySet()) {
            ...
            if (partSide == EMFCompareConstants.ANCESTOR && diff instanceof ConflictingDiffElement)
                ...
                data = EMFCompareEObjectUtils.getRightElement(diff);
            if (data == null) {
                EObject matchElements = parent.findMatchFromElement(EMFCompareEObjectUtils
                        .getLeftElement(diff));
                if (matchElements instanceof Match2Elements) {
                    // data = ((Match2Elements)
            ...
            if (partSide == EMFCompareConstants.LEFT
                ...
                if (left.eContents().size() > 0)
                    visibleEditPart = findAbstractGraphicalEditPart(left.eContents().get(leftIndex));
            } else if (partSide == EMFCompareConstants.RIGHT
                ...
                if (right.eContents().size() > 0)
                    visibleEditPart = findAbstractGraphicalEditPart(right.eContents().get(
                            rightIndex));
            }
            ...
            // CMOT 11.02.2010 this kind seems not to exist any more
            ...
            // else
            wrappedItem = new ModelContentMergeTabObject(actualEditPart, actualEditPart, color,
                    backgroundColor);
    ...
    public ModelContentMergeTabObject getUIElement(EObject data) {
        /*
         * If the diff is hidden by another (diff extension), the item won't be returned Same goes
         * for diffs that couldn't be matched
         */
        ...
        for (DiffElement diffElement : diffElements) {
            ...
            if (partSide == EMFCompareConstants.ANCESTOR
                ...
                if (diffElement instanceof DiffGroup
                    ...
                    editPart = findAbstractGraphicalEditPart(EMFCompareEObjectUtils
                            .getRightElement(parent.findMatchFromElement(EMFCompareEObjectUtils
                                    .getLeftElement(diffElement))));
    ...
    /**
     * Scrolls to, zooms to and highlights the selected element. The marker figure is drawn on the
     * scalable feedback layer.
     * 
     * @param editPart
    ...
    private void markEditParts(AbstractGraphicalEditPart editPart) {
        ...
        if (prefZoomToElement) {
            ...
            if (editPart.equals(getContents())) {
                /*
                 * if DiagramEditPart selected, zoom to fit all and not to the bounds
                 */
            ...
            /* this is SCROLLING */
            Rectangle newZoomLocation = new Rectangle(figBounds.getCenter().translate(
                    viewport.getSize().scale(-0.5)), viewport.getBounds().getSize());
        ...
        if (editPart.getParent() != null
            ...
            if (((AbstractGraphicalEditPart) editPart.getParent()).getFigure() instanceof ResizableCompartmentFigure) {
                ResizableCompartmentFigure parentFigure = (ResizableCompartmentFigure) ((AbstractGraphicalEditPart) editPart
                        .getParent()).getFigure();
                AnimatableScrollPane asp = (AnimatableScrollPane) parentFigure.getScrollPane();
                if (!asp.isExpanded()) {
     ...
     * 
     * @param target
     *            The EObject to which the corresponding EditPart should be found
     * @return The EditPart for the provided EObject
     */
    private AbstractGraphicalEditPart findAbstractGraphicalEditPart(EObject target) {

        GraphicalEditPart gepBegin = (GraphicalEditPart) getContents();
        if (target == null || gepBegin == null)
            return null;

        AbstractGraphicalEditPart foundEditPart = (GraphicalEditPart) gepBegin.findEditPart(null,
                target);

        if (foundEditPart == null) {
            GraphicalEditPart gepParent = (GraphicalEditPart) gepBegin.findEditPart(null, target
                    .eContainer());
            foundEditPart = (AbstractGraphicalEditPart) findConnectionEditPart(gepParent, target);
        }
        return foundEditPart;
    }

    /*
     * Recursive helper function. Finds an ConnectionEditPart given a starting EditPart and an
     * EObject
     */
    private AbstractGraphicalEditPart findConnectionEditPart(AbstractGraphicalEditPart epBegin,
            EObject theElement) {
        ...
        if (epBegin instanceof GraphicalEditPart) {
            ...
            while (childLI.hasNext()) {
                AbstractGraphicalEditPart epChild = (AbstractGraphicalEditPart) childLI.next();

                AbstractGraphicalEditPart elementEP = findConnectionEditPart(epChild, theElement);
                if (elementEP != null) {
                    return elementEP;
                }
            }
    ...
    /**
     * Helper function used when translating the location for a Figure to another Figure layer.
     * Needed as the feedback layer uses absolute coordinates in contrast to the relative
     * coordinates of every element's hierearchy.
     * 
     * @param from
     *            The Figure which should be translated
     * @param to
     *            The Figure with the coordinate system to which the given Figure should be
     *            translated
     * @return The translated Bounds of the <i>from</i> Figure now in the coordinate system of the
     *         <b>to</b> Figure
     */
    ...
    /**
     * Generates a Figure which should act as a ToolTip for the provided EditPart, indicating the
     * changes of the EditPart.
     * 
     * @param changedEditPart
     *            The EditPart for which the ToolTip should be created
     * @param highlightColor
     *            The color that should be used for the ToolTip, indicating the type of change
     * @return A ToolTip Figure with a String of changes a label
     */
    private IFigure getToolTip(AbstractGraphicalEditPart changedEditPart, Color highlightColor) {

        DiffElement diffElement = dataToRecursivelyDiff.get(((View) changedEditPart.getModel())
                .getElement());
    ...
    /**
     * Updates the preferences as they are set in the plug-in's preference store.
     */
    private void updatePreferences() {
        IPreferenceStore prefs = KivikPlugin.getDefault().getPreferenceStore();
        prefRelayoutDiagram = prefs.getBoolean(PreferenceConstants.PREF_KIVIK_ENABLE_AUTO_LAYOUT);
        ...
        prefInitialZoom = prefs.getInt(PreferenceConstants.PREF_KIVIK_INITIAL_ZOOM_FACTOR) / 100.0f;
    ...
    private Color brightenColor(Color highlightColor) {
        int red = (highlightColor.getRed() + 20) > 255 ? 255 : highlightColor.getRed() + 20;
        int green = (highlightColor.getGreen() + 20) > 255 ? 255 : highlightColor.getGreen() + 20;
        int blue = (highlightColor.getBlue() + 20) > 255 ? 255 : highlightColor.getBlue() + 20;
