 ...
 * <p/>
 * Initial implementation by <a href="mailto:laurent.goubet@obeo.fr">Laurent Goubet</a>, small
 * changes from item to object to comply with the changed interface, which was changed to handle
 * diagrams.
 * 
...
public class ModelContentMergeDiffTab extends TreeViewer implements IModelContentMergeViewerTab {
    ...
    /**
     * Maps a TreeItem to its data. We're compelled to map the Tree like this because of EMF's
     * FeatureMapEntry which default TreeViewers cannot handle accurately.
     */
    ...
    public ModelContentMergeTabObject getUIElement(EObject data) {
        ...
        if (treeItem == item.getVisibleObject()) {
            ...
            result = new ModelContentMergeTabObject(item.getActualObject(), treeItem, item
                    .getCurveColor());
        }
        ...
        if (result.getActualObject() == result.getVisibleObject()) {
            if (partSide == EMFCompareConstants.LEFT
                ...
                curveY = ((TreeItem) result.getVisibleObject()).getBounds().y
                        + ((TreeItem) result.getVisibleObject()).getBounds().height / 2;
    ...
    public List<ModelContentMergeTabObject> getVisibleElements() {
        ...
        while (differences.hasNext()) {
            ...
            if (nextItem.getActualObject() instanceof Item) {
                ...
                if (visibleTreeItems.contains(nextVisibleItem.getVisibleObject())) {
                    final ModelContentMergeTabObject next = new ModelContentMergeTabObject(nextItem
                            .getActualObject(), nextVisibleItem.getVisibleObject(), nextItem
                            .getCurveColor(), nextVisibleItem.getCurveY(), nextVisibleItem
                            .getCurveSize());
                    result.add(next);
                }
            }
    ...
    /**
     * Ensures the first element of the given list of items is visible in the tree, and sets the
     * tree's selection to this list.
     * 
     * @param items
    ...
    /**
     * Returns a visible ancestor of the given item. Will return the first container that is not
     * expanded.
     * 
     * @param item
    ...
    /**
     * This will map this TreeViewer's TreeItems to their data. We need to do this in order to find
     * the items associated to {@link FeatureMapEntry}s since default TreeViewers cannot handle such
     * data.
     */
    ...
    /**
     * This will map all the TreeItems in this TreeViewer that need be taken into account when
     * drawing diff markers to a corresponding ModelContentMergeTabItem. This will allow us to
     * browse everything once and for all.
     */
    private void mapTreeItemsToUI() {
        ...
        for (EObject key : dataToDiff.keySet()) {
            ...
            if (visibleItem != null)
                wrappedItem = new ModelContentMergeTabObject(actualItem, visibleItem, color);
            else
    ...
    /**
     * This implementation of {@link PaintListener} handles the drawing of blocks around modified
     * members in the tree tab.
     * 
     * @author <a href="mailto:laurent.goubet@obeo.fr">Laurent Goubet</a>
     */
    class TreePaintListener implements PaintListener {
         ...
         */
        private void drawRectangle(PaintEvent event, ModelContentMergeTabObject item) {
            final Rectangle treeBounds = getTree().getClientArea();
            final Rectangle treeItemBounds = ((TreeItem) item.getVisibleObject()).getBounds();
            ...
            if (partSide == EMFCompareConstants.LEFT) {
                if (item.getCurveY() != treeItemBounds.y + treeItemBounds.height / 2) {
                    ...
                    event.gc.drawRoundRectangle(rectangleX, rectangleY, rectangleWidth,
                            rectangleHeight, rectangleArcWidth, rectangleArcHeight);
                    event.gc.setLineStyle(SWT.LINE_SOLID);
                    event.gc.drawLine(rectangleX + rectangleWidth, item.getCurveY(),
                            treeBounds.width + treeBounds.x, item.getCurveY());
                }
            } else if (partSide == EMFCompareConstants.RIGHT) {
                if (item.getCurveY() != treeItemBounds.y + treeItemBounds.height / 2) {
                    event.gc.setLineStyle(SWT.LINE_SOLID);
                    event.gc.drawLine(rectangleX + rectangleWidth, item.getCurveY(), treeBounds.x,
                            item.getCurveY());
                } else {
                    event.gc.setLineStyle(SWT.LINE_DASHDOT);
                    event.gc.drawRoundRectangle(rectangleX, rectangleY, rectangleWidth,
                ...
                if (item.getCurveY() != treeItemBounds.y + treeItemBounds.height / 2) {
                    event.gc.setLineStyle(SWT.LINE_SOLID);
                    event.gc.drawLine(rectangleX + rectangleWidth, item.getCurveY(), rectangleX,
                            item.getCurveY());
                } else {
                    event.gc.setLineStyle(SWT.LINE_DASHDOT);
                    event.gc.drawRoundRectangle(rectangleX, rectangleY, rectangleWidth,
