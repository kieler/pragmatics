...
def main(args):
    ...
    # check category file
    category = join(args.path, 'build/de.cau.cs.kieler.pragmatics.repository/category.xml')
    ...
    # check pom file
    pom = join(args.path, 'build/de.cau.cs.kieler.pragmatics.repository/pom.xml')
    if not isfile(pom):
        print 'pom.xml does not exist: ' + pom
        pause(args)
    else:
        setPomVersion(join(args.path, 'build'), 'de.cau.cs.kieler.pragmatics.repository', args)
